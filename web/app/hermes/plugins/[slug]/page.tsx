...
export function generateStaticParams() {
  return hermesPlugins.map((entry) => ({ slug: entry.id }));
}
...
export default async function HermesPluginDetailPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const jsonLd = {
    ...
    applicationCategory: 'Hermes Agent Plugin',
      ...
      <main className="flex-1">
        <Container className="py-12">
          <nav className="mb-6 flex gap-2 text-sm text-muted-foreground">
            ...
            <span>/</span>
            <Link href="/hermes/plugins" className="hover:text-foreground">Hermes Plugins</Link>
            <span>/</span>
          ...
          {related.length > 0 && (
            <section className="mt-16 border-t border-border/60 pt-10">
              <SectionLabel>Related {ecosystemLabel('hermes')} plugins</SectionLabel>
